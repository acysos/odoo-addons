...
class AccountMove(models.Model):
    ...
    def _get_default_sii_description(self):
        inv_type = self.env.context.get('move_type')
        if not inv_type:
    ...
    def onchange_fiscal_position(self):
        for invoice in self:
            if invoice.fiscal_position_id and 'out' in invoice.move_type:
    ...
    def _get_sii_description_from_lines(self):
        for invoice in self:
            ...
            header_purchase = company.sii_header_purchase or ''
            description = '/'
            if invoice.move_type in ['out_invoice', 'out_refund'] and header_sale:
                description = header_sale
            if invoice.move_type in ['in_invoice', 'in_refund'] and header_purchase:
                description = header_purchase
            if method_desc == 'auto':
                for line in invoice.invoice_line_ids:
    ...
    @api.model
    def _prepare_refund(
            self, invoice, invoice_date=None, date=None,
            description=None, journal_id=None):
        values = super(AccountMove, self)._prepare_refund(
            invoice, invoice_date, date, description,
            journal_id)
    ...
    def create(self, vals):
        if 'move_type' in vals and vals['move_type'] in [
    ...
    def write(self, vals):
        ...
        if ('name' in vals and self.move_type in ['out_invoice', 'out_refund']):
            if self.sii_sent and vals['name'] != self.name:
                raise UserError(_(
                    "This invoice is sent to SII, if you change the number"
                    " you have to cancel it and drop the invoice from SII"))
            elif vals['name'] != self.name:
                vals['sii_resend'] = True
        if ('ref' in vals and self.move_type in ['in_invoice', 'in_refund']):
            if self.sii_sent and vals['ref'] != self.ref:
                raise UserError(_(
                    "This invoice is sent to SII, if you change the number"
                    " you have to cancel it and drop the invoice from SII"))
            elif vals['ref'] != self.ref:
                vals['sii_resend'] = True
    ...
    def _get_sii_tax_line(self, tax_line, line, line_taxes):
        ...
        taxes = tax_line.compute_all(
            ...
            quantity=line.quantity, product=line.product_id,
            partner=line.move_id.partner_id)
        if self.move_type == 'out_refund' and self.refund_type == 'I':
    ...
    def _get_sii_out_taxes(self):
        ...
        # taxes_sfesisps = self._get_taxes_map(['SFESISPS'], self.invoice_date)
    ...
    def _get_sii_in_taxes(self):
        ...
        if len(taxes_isp) > 0:
            for key, line in list(taxes_isp.items()):
                if self.move_type == 'in_refund' and self.refund_type == 'I':
                    if line.get('CuotaRecargoEquivalencia', False):
                        line['CuotaRecargoEquivalencia'] = \
                            -round(line['CuotaRecargoEquivalencia'], 2)
                    if line.get('CuotaSoportada', False):
    ...
    def _send_invoice_to_sii(self):
        for invoice in self.filtered(
                lambda i: i.state in ['posted'] and i.is_sii_mapped and (
    ...
    def send_recc_payment_registry(self, move):
        for invoice in self:
            ...
            try:
                ...
                self.env['aeat.sii.result'].sudo().create_result(
                    invoice, res, 'recc', False, 'account.move')
                ...
                self.env['aeat.sii.result'].sudo().create_result(
                    invoice, False, 'recc', fault, 'account.move')
