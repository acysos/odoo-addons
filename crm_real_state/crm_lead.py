...
class crm_lead(osv.osv):
    ...
    def search_tops_suggested(self, cr, uid, ids, context=None):
        ...
        for crm_lead in self.browse(cr, uid, ids, context):
            if crm_lead.state in ['open', 'pending']:
                ...
                sug_obj.write(cr, uid, sug_ids, {'updated': False})
                domain = []
    ...
    def run_search_top_suggested(self, cr, uid, context=None):
        if not context:
            context = {}
        ids = self.search(cr, uid, [('state', 'in', ['open', 'pending'])])
        context.update({'cron': True})
        self.search_tops_suggested(cr, uid, ids, context)
        return True
    ...
    def create(self, cr, uid, vals, context=None):
        if not context:
            context = {}
        res_id = super(crm_lead, self).create(cr, uid, vals, context)
        self.search_tops_suggested(cr, uid, [res_id], context)
        return res_id
