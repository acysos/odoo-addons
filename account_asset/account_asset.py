    ...
    def compute_depreciation_board(self, cr, uid, ids, context=None):
        ...
        for asset in self.browse(cr, uid, ids, context=context):
            ...
            if asset.prorata:
                ...
                # date is last entry + method period
                depreciation_date = datetime.strptime(
                    self._get_last_depreciation_date(
                        cr, uid, [asset.id], context)[asset.id], '%Y-%m-%d')
                if posted_depreciation_line_ids:
                    last_dep_line = depreciation_lin_obj.browse(
                        cr, uid, [posted_depreciation_line_ids[0]],
                        context)[0]
                    if last_dep_line.depreciation_date:
                        last_depreciation_date = datetime.strptime(
                            last_dep_line.depreciation_date, '%Y-%m-%d').date()
                ...
                if (len(posted_depreciation_line_ids)>0) and asset.method_time != 'end':
...
class account_asset_depreciation_line(osv.osv):
    ...
    _columns = {
        ...
        'period_line': fields.boolean(string='Period Line')
    }
