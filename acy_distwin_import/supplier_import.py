...
class distwin_supplier_import(osv.osv):
    _name = "distwin.supplier.import"
    _description = "Distwin Supplier Import"
    
    _columns = {
        'name': fields.char('Import Code', size=64, select=1),
        'date': fields.date('Date', required=True, states={'confirm': [('readonly', True)]}),
        'partner_lines': fields.one2many('distwin.supplier.line', 'import_id', 'Supplier lines', states={'draft':[('readonly',False)]}, readonly=True),
    ...
    def action_confirm(self, cr, uid, ids, context=None):
        ...
        for partner_line in partners.partner_lines:
            ...
            reference = 'P'+partner_line.account_number[3:]
            ...
            if partner_id:
                ...
                account_obj.write(cr,uid,partner.property_account_payable.id,{'name':partner_line.name},context=context)
            else:
                res_ids = account_type_obj.search(cr, uid, [('code', '=', 'terceros - pay')]) # Busca tipo cuenta de usuario rec
                ...
                account_obj.write(cr,uid,partner.property_account_payable.id,{'code':partner_line.account_number},context=context)
    ...
    def action_cancel_draft(self, cr, uid, ids, context=None):
        ...
        return True
distwin_supplier_import()

class distwin_supplier_line(osv.osv):
    
    _name = "distwin.supplier.line"
    _description = "Distwin Supplier Line"
    ...
    _columns = {
        ...
        'import_id': fields.many2one('distwin.supplier.import', 'Operator registry', ondelete='cascade'),
    }
distwin_supplier_line()

class import_distwin_supplier_file_wizard(osv.osv):
    
    _name = 'import.distwin.supplier.file.wizard'

    _columns = {
        'file': fields.binary('CSV Supplier File', required=True, filename='file_name'),
        'file_name': fields.char('CSV Supplier File', size=64, readonly=True),
        }
    ...
    def import_action(self, cr, uid, ids, context=None):
        ...
        import_partner_obj = self.pool.get('distwin.supplier.import')
        partner_line_obj = self.pool.get('distwin.supplier.line')
                ...
                if fields[0] != '':
                    if fields[0] != 'codpro':
                        values = {
                            ...
                            'address_phone': fields[7].replace('"',''),
                            'address_zip': fields[5].replace('"',''),
                            'address_country': user.company_id.partner_id.address[0].country_id.id,
                            'vat_number': user.company_id.partner_id.address[0].country_id.code+fields[6].replace('"',''),
...
import_distwin_supplier_file_wizard()
