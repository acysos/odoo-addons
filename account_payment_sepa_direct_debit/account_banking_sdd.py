...
class banking_export_sdd(osv.osv):
    ...
    _columns = {
        ...
        'filename': fields.function(
            _generate_filename, type='char', size=256, method=True,
...
class sdd_mandate(osv.osv):
    ...
    _defaults = {
        ...
        self.pool.get('ir.sequence').get(cr, uid, 'sdd.mandate.reference'),
        'state': 'draft',
        'sepa_migrated': True,
    }
    ...
    def mandate_partner_bank_change(
        ...
        res = {'value': {}}
        if partner_bank_id:
            partner_bank_read = self.pool.get('res.partner.bank').read(
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if (context.get('type') == 'receivable'
            ...
            if partner_bank_id and 'sdd_mandate_id' not in vals:
                mandate_ids = self.pool.get('sdd.mandate').search(cr, uid, [
            ...
            if (payline.sdd_mandate_id and payline.bank_id
                ...
                raise osv.except_osv(
                    ...
                    _("The payment line with reference '%s' has the bank "
                        ...
                        "'%s').") % (
                        payline.name,
                        self.pool.get('res.partner.bank').name_get(
