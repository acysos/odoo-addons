...
#         return res
    @api.multi
    def _get_activity_name(self, field_name=False, arg=False):
...
#             ids = [ids]
        ...
        for activity in self:
            if activity.type == 'job' and activity.job_id:
                print activity.job_id.name_get()[0][1]
                activity.name = activity.job_id.name_get()[0][1]

            elif activity.type == 'leave' and activity.leave_id:
                print activity.leave_id.name_get()[0][1]
                activity.name = activity.leave_id.name_get()[0][1]
    ...
    name = fields.Char(compute ='_get_activity_name', string='Activity Name')
    type = fields.Selection([('leave', 'Leave'),
                ('job', 'Job')],'Activity Type', required=True)
    code = fields.Char('Code', help="Used for payslip computation")
