...
class real_estate_top(models.Model):
    ...
    @api.multi
    def xml_urbaniza(self):
        
        company = self.env.user.company_id
        
        raiz = etree.Element('Inmuebles')
        for top in self.search([('urbaniza','=',True),
                                    ('available','=',True)]):
            ...
            direccion_tipo_via.text = etree.CDATA(top.urbaniza_street_type or '')
            ...
            if top.operation=='sale':
                ...
                honorarios_agente_captador.text = str(top.urbaniza_fee_captador or '')
                honorarios_agente_colaborador = etree.SubElement(gestion1, 'honorarios_agente_colaborador')
                honorarios_agente_colaborador.text = str(top.urbaniza_fee_colaborador or '')
                comision_referido = etree.SubElement(gestion1, 'comision_referido')
                comision_referido.text = top.urbaniza_commission_refer or ''
            ...
            if top.urbaniza_exclusivity_doc:
                url_doc_exclusividad.text = 'http://' + company.domain + 'website/pdf/real.estate.top/urbaniza_exclusivity_doc/' + str(top.id) + '/urbaniza_exclusivity_doc_filename/pdf.pdf'     
            else:
            ...
            eficiencia_energetica_fecvalid.text = top.urbaniza_energy_date or ''
            ...
            poblacion_sucursal.text = company.city or ''
    ...
    urbaniza = fields.Boolean('Publicado en inmoagrupa.com')
    urbaniza_street_type = fields.Selection(STREET_TYPE, 
                                 'Tipo de calle', select=True, readonly=False)
    urbaniza_type = fields.Selection(TYPE, 'Tipo en urbaniza', 
                                              select=True, readonly=False)
    urbaniza_piso = fields.Char('Piso', size=64, 
                                             required=False, readonly=False)
    urbaniza_letra = fields.Char('Letra', size=64, 
                                             required=False, readonly=False)
    urbaniza_latitude = fields.Char('Latitud', size=64, 
                                             required=False, readonly=False)
    urbaniza_longitude = fields.Char('Longitud', size=64, 
                                             required=False, readonly=False)
    urbaniza_fee_type = fields.Selection(FEE_TYPE,'Tipo honarios', 
                                                  select=True, readonly=False)
    ...
    urbaniza_bank = fields.Selection(YESNO, 'Piso Banco', 
                                                  select=True, readonly=False)
    urbaniza_vpo = fields.Selection(YESNO, 'Piso VPO', 
                                                  select=True, readonly=False)
    urbaniza_heating = fields.Char(compute='_get_urbaniza_heating', 
                                           method=True, store=False)
    urbaniza_exclusivity_doc_filename = fields.Char("Doc Filename") 
    urbaniza_exclusivity_doc = fields.Binary('Documento Exclusividad')
    capt_date = fields.Date('Fecha de captación')
    urbaniza_capt_date = fields.Char(compute='_get_urbaniza_capt_date', 
                                           method=True, store=False)
    end_date = fields.Date('Fecha fin de mandato')
    urbaniza_end_date = fields.Char(compute='_get_urbaniza_end_date', 
                                           method=True, store=False)
    urbaniza_energy_date = fields.Char(compute='_get_urbaniza_energy_date', 
                                           method=True, store=False)
...
#         file.close()
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if 'urbaniza_exclusivity_doc' in vals:
            user = self.pool.get('res.users').browse(cr, uid, uid)
            company = user.company_id
            top = self.browse(cr,uid,res,context)
            directory = company.local_media_repository+top.name
            filename = _('exclusivity_doc_%s.pdf')%(top.name)
            if vals['urbaniza_exclusivity_doc'] != False:
                self._save_file(directory, filename, vals['urbaniza_exclusivity_doc'])
                if company.ftp_host:
                    self._upload_to_ftp_server(cr,uid, company, top, 
                                               filename, context)
                    energy_doc_url = company.url_media_repository+top.name+'/'+filename
                    self.write(cr,uid,res,{'urbaniza_exclusivity_doc_url' : energy_doc_url},
                               context)
        return res
            ...
            for top_id in ids:
                ...
                if vals['urbaniza_exclusivity_doc'] != False:
                    ...
                    if company.ftp_host:
                        self._upload_to_ftp_server(cr,uid, company, top, 
                                                   filename, context)
                        vals['urbaniza_exclusivity_doc_url'] = company.url_media_repository+top.name+'/'+filename
                elif vals['urbaniza_exclusivity_doc'] == False:
                    vals['urbaniza_exclusivity_doc_url'] = False
