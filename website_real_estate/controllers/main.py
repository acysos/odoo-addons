...
class top_view(http.Controller):
    ...
    def realestatetops(self, top, **post):
        ...
        return request.website.render("website_real_estate.tops_template", values)

    
    @http.route(['/realestate/search',
                 '/realestate/search/page/<int:page>'], type='http', auth="public", website=True)
    def searchtopslist(self, page=0, **post):
        
        url = "/realestate/search"
        values = self.searchtops(page, url, post)
        values['page'] = page
        return request.website.render("website_real_estate.search_list", values)
    
    
    @http.route(['/realestate/searchmap',
                 '/realestate/searchmap/page/<int:page>'], type='http', auth="public", website=True)
    def searchtopsmap(self, page=0, **post):
        
        url = "/realestate/searchmap"
        values = self.searchtops(page, url, post)
        values['page'] = page
        return request.website.render("website_real_estate.search_map", values)
    ...
    def featuredtops(self, page=0, **post): 
        ...
        return request.website.render("website_real_estate.featured_tops", values)
