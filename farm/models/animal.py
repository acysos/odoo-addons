...
class Tag(models.Model):
    ...
    animals = fields.One2many(
        comodel_name='farm.animal', inverse_name='tag',
        string='Animals')
...
class Animal(models.Model):
    ...
    #    string='Tags')
    tag = fields.Many2one(string='Ear Tag', comodel_name='farm.tags',)
    ...
    sex = fields.Selection([('male', "Male"),
                            ...
                            ], string='Sex', required=True, default='undetermined')
        ...
        for record in res:
            ...
            if record.origin == 'raised':
                ...
                m_line_obj.create({
                    'move_id': new_move.id,
                    'product_id': record.lot.lot.product_id.id,
                    'product_uom_id': product_tmpt.uom_id.id,
                    'location_id': raise_location.id,
                    'location_dest_id': record.initial_location.id,
                    'quantity': 1,
                    'lot_id': record.lot.lot.id,
    ...
    def create(self, vals_list):
        results = super(Animal, self).create(vals_list)
        for res in results:
            res.create_first_move(res)
    ...
    @api.depends('tag', 'number')
    def get_name(self):
        for animal in self:
            if animal.tag:
                animal.name = animal.tag.name + '-' + animal.number
            else:
...
class Female(models.Model):
    ...
    days_from_farrowing = fields.Integer(string='Unpregnat Days',
                                         compute='get_days_from_farrowing', store=True)
    ...
    def get_last_produced_group(self):
        ...
        for res in self:
            ...
            if last_farrowing_events:
                res.last_produced_group = \
                    last_farrowing_events.produced_group.animal_group.id
            else:
                res.last_produced_group = False

    def get_last_farrowing_day(self):
        FarrowingEvent = self.env['farm.farrowing.event']
        ...
        if not last_valid_farrowing:
            ...
            print(str(last_valid_farrowing[0].timestamp))
            return datetime.strptime(str(
                last_valid_farrowing[0].timestamp).split('.')[0], DFORMAT)


    @api.depends('cycles', 'cycles.farrowing_event')
    def get_days_from_farrowing(self):
        for res in self:
            last_farrowing_day = res.get_last_farrowing_day()

            if not last_farrowing_day:
                res.days_from_farrowing = False
            else:
                days_from_farrowing = (
                    datetime.today() - last_farrowing_day).days
                res.days_from_farrowing = days_from_farrowing
...
class FemaleCycle(models.Model):
    ...
    def get_days_between_weaning_and_insemination(self):
        if not self.insemination_events:
            self.days_between_wearing_and_insemination = -1
        else:
    ...
    @api.depends('farrowing_event', 'farrowing_event.cycle.live')
    def get_live(self):
        if self.farrowing_event:
            print('if')
            print(self.farrowing_event[-1].event)
            print(self.farrowing_event[-1].event.live)
            self.live = self.farrowing_event[-1].event.live
        else:
            print('else')
            self.live = 0

    @api.depends('farrowing_event', 'farrowing_event.cycle.dead')
    def get_dead(self):
        if self.farrowing_event:
            self.dead = self.farrowing_event[-1].event.dead
        else:
            self.dead = 0
    ...
    def get_lactating_days(self):
        if not self.farrowing_event or not self.weaning_event:
            self.days_between_farrowing_weaning = -1
        else:
    ...
    @api.depends('abort_event', 'diagnosis_events', 'farrowing_event')
    ...
    def get_last_produced_group(self):
        ...
        last_farrowing_events = farrowingEvent_obj.search([
            ...
            order='timestamp DESC', limit=1)
        if last_farrowing_events:
            return last_farrowing_events.produced_group.animal_group.id
        return None
