...
class EventOrder(models.Model):
    ...
    event_type = fields.Selection([
        ...
        ], string="Event Type", required=True, select=True, default='feed')
    ...
    def create(self, vals_list):
        ...
        res = super(EventOrder, self).create(vals_list)
        return res
    ...
    def get_active_events(self):
        if self.event_type == 'medication':
            ...
            print('feed_events', self.feed_events)
    ...
    def confirm(self):
        for res in self:
            active_events = res.get_active_events()
            validated_events = res.confirm_event(active_events)
            if(validated_events):
                    res.state = 'validated'
            else:
                print('error', str(active_events) + ' ' + str(validated_events))
                raise Warning(
                    _('There are no event associated with this work order'))

    def confirm_event(self, events):
        control = False
        for event in events:
