...
class RemovalEvent(models.Model):
    ...
    specific_lot = fields.Many2one(comodel_name='stock.production.lot',
                                   string='Specific lot')
    removed_tags = fields.Many2many(comodel_name='farm.tags',
                                    string='Removed Tags')
    ...
    @api.onchange('removed_tags')
    def onchange_removed_tags(self):
        if not self.animal and not self.animal_group and self.removed_tags:
            animal = self.env['farm.animal'].search([('tag', '=', self.removed_tags.id)])
            if animal:
                self.animal = animal
                self.amimal_type = animal.type
            else:
                animal = self.env['farm.animal.group'].search([('tag', '=', self.removed_tags.id)])
                if animal:
                    self.animal_group = animal
                    self.animal_type = 'group'
                else:
                    raise Warning(_('Tag not found'))
    ...
    def remove_group(self):
        ...
        if self.removed_tags:
            removed_tags_obj = self.env['farm.tags.removed']
            for tag in self.removed_tags:
                self.animal_group.tags = [(3, tag.id)]
                new_tag = removed_tags_obj.create({
                    'tag': tag.id,
                    'animal_group': self.animal_group.id,
                    })
                self.animal_group.removed_tags = [(4, new_tag.id)]
