        ...
        if partner_account:
            if checked:
                ...
                if partner_account.name != partner.name:
                    ...
                    partners = self.pool.get('ir.property').search(cr, uid, [('value_reference','=',value),('res_id','=',res_id)], context=context)
                    if len(partners) == 1:
                        default_account = self.pool.get('ir.property').search(cr, uid, [('value_reference','=',value),('res_id','=',None)], context=context)
