...
class stock_picking(osv.osv):
    ...
    def action_invoice_create(self, cr, uid, ids, journal_id=False,
        ...
        for picking in self.browse(cr, uid, ids, context=context):
            ...
            sequence += 1
            context['lang'] = picking.partner_id.lang
            lang_obj = self.pool.get('res.lang')
            lang_id = lang_obj.search(cr,uid,[('code','=',context['lang'])], context=context)
            lang = lang_obj.browse(cr, uid, lang_id, context)[0]
            invoice_line_id = invoice_line_obj.create(cr, uid, {
                'name': _('Picking List %s - %s') % (picking.name, time.strftime(lang.date_format,time.strptime(picking.date_done,"%Y-%m-%d %H:%M:%S"))),
                'state': 'title',
