...
class banking_export_pain(osv.osv_memory):
    _name = 'banking.export.pain'

    def _validate_iban(self, cr, uid, iban, bank_id, context=None):
        '''if IBAN is valid, returns IBAN
        if IBAN is NOT valid, raises an error message'''
        partner_bank_obj = self.pool.get('res.partner.bank')
        if partner_bank_obj.check_iban(cr, uid, [bank_id], context=context):
            return iban.replace(' ', '')
        else:
    ...
    def generate_party_block(
            ...
            bank_id, eval_ctx, gen_args, context=None):
        ...
        piban = self._prepare_field(
            ...
            gen_args=gen_args,
            context=context)
        viban = self._validate_iban(cr, uid, piban, bank_id, context=context)
