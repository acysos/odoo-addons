...
class PayrollImportXls(models.Model):
    ...
    def _do_import_nominasol(
        ...
        payroll_extid = worksheet.cell_value(curr_row, number_col)
        ...
        debit_deduction = worksheet.cell_value(curr_row, col_deduction)
        deduction_vals = {'account_id': company.account_deduction.id,
                   'name': _('Payroll'),
                   'journal_id': journal.id,
                   'debit': 0,
                   'credit': debit_deduction,
                   'ref': _('Payroll')
                   }
        move_vals['line_ids'] = [
            (0, 0, line64_vals), (0, 0, irpf_vals), (0, 0, total_vals),
            (0, 0, ss_vals), (0, 0, deduction_vals)]
        try:
